...
class PDEOperatorRBFCartesianProd(Data):
    ...
    def __init__(
        ...
        self.Gx_train, self.Gy_train, self.Gz_train, self.Lap, self.ind_i = self.get_G_matrix() 
        self.Gx_test, self.Gy_test, self.Gz_test, self.Lap , self.ind_i = self.get_G_matrix()
    ...
    def _losses(self, outputs, loss_fn, inputs, model, num_func, training):    # only run when compiling, tensor, inputs:self.net.inputs
        ...
        for i in range(num_func):
            ...
            out = outputs[i][:, None]
            f = self.pde.pde(inputs[1], out, aux_vars[i], Gx, Gy,Gz, self.Lap, self.ind_i, pde_aux_vars) if self.pde.pde is not None else []               
            ...
            if i < num_func_bc:
                error = bkd.tf.reshape(out[beg:end], (bc.values.shape[1],))- bc_values_reshaped[i]                 #bc.values.shape - N_ob_k, N_ob_p
    ...
    def losses_test(self, targets, outputs, loss_fn, inputs, model, aux=None):
        beg, end = 0, self.pde.num_bcs[0] # 0, 2500
        losses = []
        num_func = self.test_x[0].shape[0] # number for testing
        for i in range(num_func):
            # compute the loss for pde residual
            out = outputs[i][:, None]
            f = []
            if self.pde.pde is not None:
                f = self.pde.pde(self.anchors_test[1], out, self.test_aux_vars[i], self.Gx_test, self.Gy_test, self.Gz_test, self.Lap, self.ind_i, self.pde.test_aux_vars[-len(self.pde.train_x_all):,:])
    ...
    def test(self):
        ...
        if self.num_test is None and (self.anchors_test is None):
            ...
            if self.anchors_test is not None:
                ...
                vx_ob = np.loadtxt(f"{dname}kappast_test.txt",delimiter=",")[:len(self.anchors_test[0]), :]
                vx = np.vstack((vx_ob, vx))
    ...
    def get_G_matrix(self):
        ...
        Gx_train = np.loadtxt(f"{dname}Gx.txt", delimiter=",")
        Gy_train = np.loadtxt(f"{dname}Gy.txt", delimiter=",")
        Gz_train = np.loadtxt(f"{dname}Gz.txt", delimiter=",")
        Gx_train = tf.cast(Gx_train, tf.float32)
        Gy_train = tf.cast(Gy_train, tf.float32)
        Gz_train = tf.cast(Gz_train, tf.float32)
        Lap = np.loadtxt(f"{dname}Lap.txt", delimiter=",")
        Lap = tf.cast(Lap, tf.float32)
        ind_i = np.loadtxt(f"{dname}index_I.txt", delimiter=",")
        return Gx_train, Gy_train, Gz_train, Lap, ind_i  # for one kappa , G: (N_trainx , N_trainx), G_train: (Nk, N_trainx , N_trainx)
