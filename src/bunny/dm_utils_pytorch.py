...
def scipy_csr_to_torch_sparse(csr_mat):
    ...
    v_torch = torch.tensor(values, dtype=torch.float64)
...
class PDEOperatorDMCartesianProd(Data):
    ...
    def _build_rhs_constant(self, x):
        """
        f = 0.1 * (sum of x[:,:3])^2. Return a torch tensor of shape (N,1).
        """
        sum_xyz = np.sum(x[:, :3], axis=1, keepdims=True)
        f_np = 0.1 * (sum_xyz**2)
        return torch.tensor(f_np, dtype=torch.float64)

    def _build_pde_operators_train(self):
        """
        Precompute the matrix M_i = -diag(kappa_i)*L + I
        """
        self.operator_train = []
        for i in range(self.M):
            ...
            kappa_tensor = torch.tensor(kappa_i, dtype=torch.float64, device="cpu")
    ...
    def _build_pde_operators_test(self):
        """
        Similar operator building for test set if needed.
        """
        ...
        for i in range(M_test):
            ...
            row_indices = L_torch.indices()[0, :]
            kappa_tensor = torch.tensor(kappa_i, dtype=torch.float64)
            kappa_values = torch.gather(kappa_tensor, 0, row_indices, device="cpu")
            val = L_torch.values() * -kappa_values
            L_mod = torch.sparse_coo_tensor(
                L_torch.indices(), val, size=L_torch.shape, device="cpu"
            ).coalesce()
    ...
    def _losses(self, outputs, loss_fn, inputs, model, num_func, training):
        ...
        for i in range(num_func):
            r_i = torch.sparse.mm(operators[i].to(outputs.device), outputs[i].unsqueeze(-1))- f_const 
            pde_losses.append(loss_fn(r_i, torch.zeros_like(r_i)))
        pde_loss = torch.mean(torch.stack(pde_losses,0))

        num_func_bc = self.bc.values.shape[0]
        bc_values_tensor = torch.tensor(self.bc.values[:num_func_bc], dtype=torch.float64, device=outputs.device)
        bc_errors = outputs[:num_func_bc] - bc_values_tensor
        bc_loss = loss_fn(bc_errors, torch.zeros_like(bc_errors)).mean()
    ...
    def losses_test(self, targets, outputs, loss_fn, inputs, model, aux=None):
        ...
        losses_all = []
        num_func_test = self.anchors_test[0].shape[0]

        for i in range(num_func_test):
            diff = outputs[i] - torch.tensor(self.anchors_test_y[i], dtype=torch.float64)
            losses_all.append(loss_fn(diff, torch.zeros_like(diff)))
        return [torch.mean(torch.stack(losses_all)), torch.mean(torch.stack(losses_all))]
...
class DiffusionMap:
    ...
    def get_matrix(self, kappa_i):
        ...
        L = L - I
        L = L / self.epsilon
        return L.astype(np.float64).tocsr()
