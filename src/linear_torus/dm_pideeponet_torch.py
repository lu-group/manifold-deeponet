...
def main(Numk, n, convs, D, w, fn):
    ...
    model.compile("adam", lr=0.001, decay=("inverse time", 1000, 0.05),
        metrics=["mean l2 relative error"], loss_weights=[w, 1.0])
    ...
    dde.utils.save_loss_history(losshistory, f"{OUTPUT_DIR}/loss_dm_pi_100_{Numk}_{n}_{D}_{convs}_{w}_{fn}.dat")
    dde.utils.plot_loss_history(losshistory, f"{OUTPUT_DIR}/loss_dm_pi_100_{Numk}_{n}_{D}_{convs}_{w}_{fn}.png")
    ...
    np.savetxt(f"{OUTPUT_DIR}/pred_u_dm_100_{Numk}_{n}_{D}_{convs}_{w}_{fn}.txt", pred_u, delimiter=",")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--w",      type=float, default=1e-2)
    parser.add_argument("--fn",      type=str, default="0")
    
    args = parser.parse_args()
    print("args:", args)
    main(args.numk, args.n, args.convs, args.D, args.w, args.fn)
