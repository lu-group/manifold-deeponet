...
from rbf_utils_pi import PDEOperatorRBFCartesianProd, KappaFuncSpace
...
def load_data():
    d = np.load(f"{DNAME}/torus_data_train1_rbf.npz", allow_pickle=True)
    X_train_1, X_train_2, y_train = d["X_train_branch"], d["X_train_trunk"], d["y_train"]
    X_train = (X_train_1, X_train_2)

    d = np.load(f"{DNAME}/torus_data_test1_rbf.npz", allow_pickle=True)
    X_test_1, X_test_2, y_test = d["X_test_branch"], d["X_test_trunk"], d["y_test"]
...
def main(Numk, fn):
    ...
    N_k, N_p = 100, 2500
    anchors_train = load_train_data(f"{DNAME}/torus_data_train1_rbf.npz", N_k, N_p)[0]

    # 3) PDE test anchors
    N_kt, N_pt = 100, 2500
    anchors_test_data = load_test_data(f"{DNAME}/torus_data_test1_rbf.npz", N_kt, N_pt)

    # 4) Observation (BC) data
    N_ob_k, N_ob_p = Numk, 2500
    anchors_train_ob, anchors_train_ob_y = load_train_data(
        f"{DNAME}/torus_data_train1_rbf.npz", N_ob_k, N_ob_p
    )
    ...
    dde.utils.save_loss_history(losshistory, f"{OUTPUT_DIR}/loss_rbf_pi_100_{Numk}_{fn}.dat")
    dde.utils.plot_loss_history(losshistory, f"{OUTPUT_DIR}/loss_rbf_pi_100_{Numk}_{fn}.png")
    ...
    np.savetxt(f"{OUTPUT_DIR}/pred_u_rbf_100_{Numk}_{fn}.txt", pred_u, delimiter=",")
