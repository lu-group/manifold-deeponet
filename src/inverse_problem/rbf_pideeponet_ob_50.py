...
from rbf_utils_50 import PDEOperatorRBFCartesianProd, KappaFuncSpace
dde.config.disable_xla_jit()

m=50
sname = "_rbf"
folder = "../../data/data_inverse/data_50"
fname_train = f"{folder}/inverse_torus_train{sname}.npz"
fname_test = f"{folder}/inverse_torus_test{sname}.npz"
...
def main(fn):
    ...
    def ex_func(inputs):
        a,b = 1, 1
        x = np.loadtxt(f"{folder}/tp_train.txt", delimiter=",")
        t,p = x[:,0:1], x[:,1:2]
    ...
    losshistory, train_state = model.train(iterations=200002, batch_size=None, display_every = 1000, model_save_path="model/model500")
    ...
    np.savetxt(f"rbf_res/rbf_pred_u_50_{fn}.txt", pred_u, delimiter=',')
