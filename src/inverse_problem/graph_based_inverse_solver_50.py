...
from pideeponet_predict_50 import predict_model, predict_u
...
def BEIP_T_DeepONet(m,NN,sigma,beta,tau,gamma,obs_set,Nexp, M, is_orig):
    ...
    Q = np.loadtxt("Q_50.dat", delimiter=",")
...
if __name__ == "__main__":
    m=50    #number of points on manifold
    NN=16   #NN = number of neighbors when constructing graph Laplacian
    sigma, beta = 0.01, 0.02  #sigma-noise level, beta-step size of pCN
    ...
    np.savetxt("results/res_list_50_50_{}_{}.txt".format(M,no), res_list)
    np.savetxt("results/theta_pm_sd_50_50_{}_{}.txt".format(M,no), np.hstack((PM,Var.reshape((-1, 1)))))
    np.savetxt("results/pm_u_50_50_{}_{}.txt".format(M,no), p_pm)
    np.savetxt("results/cu_50_50_{}_{}.txt".format(M,no), np.hstack((c,u)))
    print(e)
    print("the relative noise level:", sigma*np.sqrt(m**2)/np.linalg.norm(u))
