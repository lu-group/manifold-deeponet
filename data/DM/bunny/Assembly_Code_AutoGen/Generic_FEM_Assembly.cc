...
void GFA::Setup_Data(const mxArray *prhs[]) // input from MATLAB
{
    ...
    Base_Matrix_RHS = new Base_RHS_Data_Type(&Vh_phi_restricted_to_Gamma, &RHS_Gamma_col_constant_phi);
    ...
    Base_Matrix_RHS->geom_Gamma_embedded_in_Gamma_restricted_to_Gamma = &geom_Gamma_embedded_in_Gamma_restricted_to_Gamma;
    Base_Matrix_RHS->Vh_phi_restricted_to_Gamma = &Vh_phi_restricted_to_Gamma;
...
void GFA::Assemble_Matrices ()
{
    ...
    for (std::vector<unsigned int>::iterator DoI_Ind = Domain_Gamma_embedded_in_Gamma_restricted_to_Gamma.Sub_Assem_List.begin();
        ...
        Base_Matrix_RHS->Tabulate_Tensor(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma);
