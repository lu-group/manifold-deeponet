...
// FE matrix is:
//
// \int_{Gamma}  (v_v1_t1*(geomGamma_X1 + geomGamma_X2 + geomGamma_X3)^2)/10
//
/*------------   END: Auto Generate ------------*/
...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int i = 0; i < ROW_NB; i++)
        ...
        for (unsigned int qp = 0; qp < NQ; qp++)
            {
            const double  integrand_0 = ((*Vh_phi_restricted_to_Gamma->Func_f_Value)[i][qp].a*pow(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[0]+geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[1]+geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[2],2.0))/1.0E+1;
