...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_Gamma_embedded_in_Gamma_restricted_to_Gamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        {
        const double  t2 = p_coef_restricted_to_Gamma->Func_f_Value[0][qp].a-cos(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[1]*3.141592653589793*2.0)*sin(geom_Gamma_embedded_in_Gamma_restricted_to_Gamma->Map_PHI[qp].v[0]*3.141592653589793*2.0);
        const double  integrand_0 = t2*t2;
