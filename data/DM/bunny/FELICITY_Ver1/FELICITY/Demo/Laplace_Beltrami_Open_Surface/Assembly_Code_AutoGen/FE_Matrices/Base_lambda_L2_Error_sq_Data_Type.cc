...
void SpecificFEM::Tabulate_Tensor(const CLASS_geom_dGamma_embedded_in_Gamma_restricted_to_dGamma& Mesh)
{
    ...
    for (unsigned int qp = 0; qp < NQ; qp++)
        ...
        const double  t6 = sin(t4);
        const double  t7 = sin(t5);
        const double  t8 = cos(t4);
        const double  t9 = cos(t5);
        const double  t10 = lambda_h_restricted_to_dGamma->Func_f_Value[0][qp].a-3.141592653589793*1.0/sqrt(t2+t3+1.0)*1.0/sqrt(t2+t3+t2*t3*4.0)*(geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*t6*t7-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*t8*t9+geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[1]*t2*t6*t7*2.0-geom_dGamma_embedded_in_Gamma_restricted_to_dGamma->Map_PHI[qp].v[0]*t3*t8*t9*2.0)*2.0;
